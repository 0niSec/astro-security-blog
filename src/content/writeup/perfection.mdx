...
description: "The writeup for the Hack the Box machine Perfection. This machine is an easy rated Linux box that involves exploiting a Server-Side Template Injection (SSTI) vulnerability to get a reverse shell and then cracking a hash to get root."
pubDate: "5/24/2024"
heroImage: "../../assets/images/Perfection.png"
heroAlt: "The Perfection machine card on Hack the Box"
...
-   You can pipe `linpeas` output to `tee` and an outfile with `./linpeas.sh | tee linpeas.out`
    -   Then you can read the file with `less linpeas.out`
